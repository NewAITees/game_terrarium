...
function buildCityTrafficAnalysis(snapshot: any): any {
  ...
  const stoppedCars = cars.filter((car: any) => car.speedNow <= 0.1).length;
  const slowCars = cars.filter((car: any) => car.speedNow > 0.1 && car.speedNow < car.baseSpeed * 0.5).length;
  const movingCars = cars.length - stoppedCars;
  const avgSpeed = average(cars.map((car: any) => car.speedNow || 0));
  const avgBaseSpeed = average(cars.map((car: any) => car.baseSpeed || 0));
  const ewGreen = intersections.filter((inter: any) => inter.state === 2 || inter.state === 3).length;
  const nsGreen = intersections.filter((inter: any) => inter.state === 0 || inter.state === 1).length;
  const signalSkew = intersections.length ? Math.abs(ewGreen - nsGreen) / intersections.length : 0;
  const pressureRows = intersections
    .map((inter: any) => {
      const queue = cars.filter((car: any) => {
        const isEW = car.heading === 'E' || car.heading === 'W';
        if (isEW && car.roadIndex !== inter.zi) return false;
        if (!isEW && car.roadIndex !== inter.xi) return false;
        const axis = isEW ? inter.x : inter.z;
        return Math.abs(car.pos - axis) <= (config.roadW || 0) + (config.carLen || 0) * 1.5;
      });
      const blocked = queue.filter((car: any) => car.speedNow <= 0.1).length;
      return {
        id: inter.id,
        queue: queue.length,
        blocked,
    ...
    .sort((a: any, b: any) => b.queue - a.queue || b.blocked - a.blocked)
    .slice(0, 5);
  ...
  return {
    ...
    highlights: pressureRows.map((row: any) => `${row.id}:${row.queue} queue/${row.blocked} blocked`),
    details: {
...
function buildMossAnalysis(snapshot: any): any {
  ...
  const avgPacketPhase = average(packets.map((packet: any) => packet.t || 0));
...
function buildSmallWorldAnalysis(snapshot: any): any {
  const nodes = Array.isArray(snapshot.nodes) ? snapshot.nodes : [];
  const edges = Array.isArray(snapshot.edges) ? snapshot.edges : [];
  const packets = Array.isArray(snapshot.packets) ? snapshot.packets : [];
  const activeEdgeKeys = Array.isArray(snapshot.activeEdgeKeys) ? snapshot.activeEdgeKeys : [];
  ...
  const serverPacketShare = packets.length ? packets.filter((packet: any) => packet.toId === snapshot.serverNodeId).length / packets.length : 0;
...
function buildEscortAnalysis(snapshot: any): any {
  ...
  const enemyPressure = enemies.reduce((sum: number, enemy: any) => {
  ...
  const nearKingEnemies = enemies.filter((enemy: any) => Math.hypot((enemy.x || 0) - (king.x || 0), (enemy.z || 0) - (king.z || 0)) < 18).length;
  const readyUnits = units.filter((unit: any) => (unit.windupTimer || 0) <= 0 && (unit.fireTimer || 0) <= 0.2).length;
...
export async function startServer(getElectronState: () => any, electronDispatch: ElectronDispatch): Promise<void> {
  ...
  const networkSmallWorld = new NetworkSmallWorldRuntime();
  const cityTraffic = new CityTrafficRuntime();
