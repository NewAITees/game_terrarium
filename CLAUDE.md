...
**Champions carry their model, and reach the player on promotion.** Each ledger row stores the trained model from its best-scoring repeat under `logs/rl-research/models/<id>.json`, so acting on a champion does not mean retraining and hoping. The search publishes each promotion to the live model file itself; `npm run research:publish -- --game=<id>` republishes on demand, and `npm run research -- --no-publish` keeps a run purely exploratory. Automatic publication does not lower the bar — promotion still requires the challenger's bootstrap lower bound to clear the champion's median — it only stops a result from sitting in the ledger unused.

This used to be a deliberate manual step, on the reasoning that an unattended search should not change what the player shows. What made it safe to automate was removing the failure it was guarding against: the page now builds its agent from the encoding stored with the model (`specFromSave` in `apps/gunship/gunship.ts`), so a champion trained under any declared observation or action variant runs as-is instead of being silently rejected and falling back to an untrained agent. Publication still refuses on artifact problems — a champion with no stored model, or a model path escaping the ledger directory.

**What may be edited to widen the search, and what may not.** `scripts/rl/research_contract.ts` names both surfaces and checks the invariants that make results comparable — it runs before every search and a violation aborts the run rather than writing rows that quietly mean nothing. New hypotheses go in the variant tables (`OBSERVATION_FIELDS`, `ACTION_SETS`), the reward channels, and the search-space declaration.

The seed split, the promotion statistics, the ledger and the runner's train-then-evaluate ordering are off-limits, and that is **enforced by content hash**, not by the list: `scripts/rl/protected_surface.ts` seals them and the contract refuses to run on a mismatch. A list is documentation, and an unattended loop cannot read documentation — an edit here changes nothing about the output while changing what every number means. The seal is not a lock; these files are meant to improve. Changing one means reading the drift the tool prints and then running `npm run research:seal`, which is also the marker for where in the ledger the rules changed.
