...
**Champions carry their model, and reach the player only when told to: `npm run research:publish -- --game=<id>`.** Each ledger row stores the trained model from its best-scoring repeat under `logs/rl-research/models/<id>.json`, so acting on a champion does not mean retraining and hoping. Publication is a separate, deliberate command — the search runs unattended for hours, and changing what the player shows should not happen while nobody is looking. It refuses when the champion's observation or action variant is not the one the live page constructs: that table would be rejected on load and the page would fall back to an untrained agent, which looks exactly like a bad champion.
