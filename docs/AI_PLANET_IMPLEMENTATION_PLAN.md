...
> ステータス: 初回プロトタイプ実装後の履歴文書
>
> `apps/planet-strategy/` はすでに存在し、当初の Phase 1 / Phase 2 の大半は完了している。
> 現在の差分管理は `PLANET_TODO.md` を正とし、この文書は「最初にどう切ったか」の参照用として扱う。
...
- [x] 採掘・輸送・工場生産の基本ループ
- [x] 輸送航路の可視化
- [x] 観察 HUD / event log / telemetry
- [x] 性格別 AI 分割
- [x] 8分完結ルールへの再設計
- [x] 介入 UI と終了サマリ
- [x] 戦闘・占領の基本導入

このため、以下の「今回は入れない」「ファイル構成案」「実装順序」は**当時の実装前提のまま**であり、現状仕様では一部一致しない。
