...
- レベルアップ表示後3秒以内なら、候補から直接選択して介入する
- 未介入時に使うレベルアップ候補の解放範囲と報酬設計を調整する
- 出撃前に恒久装備と学習方針を設定する
- 研究資源を恒久アップグレードに使う
...
レベルアップ候補は3秒間表示し、その間は人間のクリックを優先する。時間切れの場合だけ
RLが強化を選び、その選択を強化方策の学習対象にする。人間の選択は戦闘状態には反映するが、
RL自身が選んだ教師データとしては記録しない。
...
OPTION系アップグレードは序盤の基本ビルドを圧迫しないよう、Wave 5到達後に初めて
レベルアップ候補へ追加する。候補は出撃中の機体アーキタイプに対応するものだけを提示し、
他機体用OPTIONは抽選しない。初回取得で1機を解禁し、追加取得で最大3機まで増加する。
上限到達後は機数ではなく、射撃間隔、追従性能、回転速度などを強化する。

| 機体 | 専用OPTION | 挙動 | 強み | 弱み |
|---|---|---|---|---|
| INTERCEPTOR | 編隊僚機 | 自機の左右に隊形を保って飛び、自機の攻撃方向へ追従射撃する | 正面への集中火力が高い | 急旋回時は隊形と照準が遅れて一時的に精度が落ちる |
| STRAFER | 追尾機 | 自機の移動軌跡を少し遅れて追い、近い別目標も攻撃する | 側面攻撃と敵弾迎撃に強い | 即応性が低く、単一目標への集中火力は控えめ |
| TURRET | 周回ドローン | 自機の周囲を旋回し、接近した敵と敵弾を自動攻撃する | 全方向防御と接近戦に強い | 遠距離の一点集中攻撃には向かない |

OPTION本体は敵や敵弾との接触対象にできるが、失うと成長選択が無駄になるため完全破壊は
させない。耐久値が尽きた場合は一定時間停止した後に復帰する。OPTIONの機数増加は弾数と
描画負荷へ直結するため最大3機を基本上限とする。
...
戦闘世界は`1600 × 900`の固定座標とし、ウィンドウサイズから分離する。ウィンドウを
縮小しても機体、敵、弾、射程、移動速度、境界位置を拡大縮小しない。表示領域が世界より
小さい場合はカメラが自機を追跡し、世界端ではカメラを停止して境界を残す。表示領域が
世界より大きい場合は世界全体を中央配置する。背景にはカメラより遅く移動する星空を描く。

UIパネルは通常時から半透明とし、自機の画面座標と重なったパネルだけ透過度をさらに上げて
自機を確認可能にする。小画面ではHP、Wave、機体種を常時表示し、進行、AI、武器の詳細は
左右端の引き出しへ格納する。アップグレード3択は操作に必要なため小画面でも常時前面へ出す。
...
試験Wave突破後にチェックポイントを置き、強化選択、HPの一部回復、
戦闘統計の短い表示を行う。3秒間の人間介入がなければRLエージェントが選ぶ。
プレイヤーの優先タグは候補生成や報酬への介入として扱う。
...
| Laser | 機体別の持続直線攻撃 | 射程、太さ、照射間隔 | 照準方向の維持と敵弾迎撃 |
| Ricochet | 敵・画面端で反射 | 反射回数、速度、威力 | 敵配置と画面端の利用 |
| Trail | 移動航跡へ残留領域 | 持続時間、範囲、設置間隔 | 移動経路と敵の誘導 |

Laserは機体によって照射形式を変える。INTERCEPTORは機首前方90度の扇形へ、波紋のように
外側へ広がるパルスレーザーを発射する。扇形内の敵と敵弾を処理できる代わりに、
一点へ照射するTURRETのレーザーより単体ダメージを低くする。
STRAFERは
機体の左右へ波打って進むウェーブ攻撃を放ち、TURRETは独立砲塔の向きへ照射する。
STRAFERのウェーブは表示上の波形に沿って敵と敵弾へ接触判定を行う。MULTI-SHOTが瞬間的な複数弾で
命中範囲を増やすのに対し、Laserは照準方向を一定時間維持することで敵と敵弾を線上処理する。
Ricochetは敵を直接照準せず、自機の攻撃軸を中心とした扇形のランダム角度へ発射する。
敵への命中後も別の敵へ追尾せず、接触面の法線を使って物理反射する。RicochetとTrailは
全機体共通だが、前者は画面端との位置関係、後者は自機の移動軌跡を
戦術資源にするため、単純なDPS強化とは異なる学習対象を作る。
...
- プレイヤーのタグ優先順位は候補出現率または報酬ウェイトを調整し、3秒間の直接選択も可能
...
17. レベルアップ候補は3秒間の人間介入を優先し、未選択時だけRLの高レベル行動として戦果を還元する。
