...
- [x] 破壊エフェクト（船: フラッシュ+デブリパーティクル / 工場: 赤フラッシュ+縮小アニメーション）
- [x] 背景星のY方向分散（addStarField の y スケール拡大済み）
- [x] 攻撃の可視化（攻撃ルートを敵対色で強調表示・HUD empire-intent に攻撃目標ラベルを約25秒保持）
...
### 視覚 — `planet_strategy_render_*.ts`

- [ ] **輸送船の複数表示** — 同一ルートに複数の輸送船が存在する場合、複数描写する（惑星上に突然出現しない）
- [x] **破壊エフェクト** — 船・工場が破壊されたとき爆発パーティクルまたはフラッシュ
- [x] **背景星のY方向分散** — 現状ほぼ水平面にまとまっている → verticalRange を大幅に広げる

### ゲームロジック — `planet_strategy.ts` / AI ファイル

- [ ] **船団単位の移動** — 攻撃・防衛船はまとまって行動させる（個別ではなくフリート単位）
- [x] **攻撃の可視化改善** — 攻撃中の船のルートラインを強調表示、どこを攻撃中か HUD に表示
