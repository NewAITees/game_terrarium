...
function runAgentRules(agent, snap) {
  for (const rule of agentRules[agent.rank] ?? []) {
    ...
    if (execAction(agent, rule.action, snap)) return;
  }
...
function updateHud() {
  document.getElementById('time').textContent = Math.floor(game.elapsed);
  document.getElementById('score').textContent = game.score;
  document.getElementById('credits').textContent = Math.floor(game.credits);
  document.getElementById('wave').textContent = game.wave;
  document.getElementById('kills').textContent = game.kills;
  document.getElementById('health').textContent = Math.max(0, Math.round(topo.server.hp));
...
function updateSeniorStrategy(dt) {
  ...
  const nextRule = callLLM(snapshot);
  game.nextScan = 6;
