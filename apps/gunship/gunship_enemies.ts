...
export type Enemy = { id: number; kind: 'ship' | 'chaser' | 'diver' | 'mine' | 'dreadnought'; x: number; y: number; vx: number; hp: number; maxHp: number; cooldown: number; radius: number };
...
export function stepEnemies(enemies: Enemy[], shots: EnemyShot[], ship: GunshipBody, dt: number): void {
  for (const enemy of enemies) {
    ...
    if (enemy.cooldown <= 0 && enemy.kind !== 'mine') {
