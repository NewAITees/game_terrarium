...
export function stepEnemies(
  ...
  for (const enemy of enemies) {
    ...
    else if (enemy.kind === 'mine') { enemy.x += enemy.vx * dt; enemy.y += Math.sin((enemy.id + simulationTime) * .7) * 13 * dt; if (enemy.x > 1140 || enemy.x < 60) enemy.vx *= -1; if (Math.hypot(enemy.x - ship.x, enemy.y - ship.y) < 82) { ship.hp -= 16 * dt; } }
