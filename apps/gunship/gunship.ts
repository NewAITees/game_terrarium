...
void reloadLiveModel().finally(() => {
  if (document.documentElement.dataset.rlModelStatus === 'loading') { document.documentElement.dataset.rlModelStatus = 'fallback'; rlHud.update({ modelState: 'no-model' }); }
});
...
let currentDecision = agent.decide(core.ship, core.enemies, core.enemyShots, core.orbs, { weapon: weaponKind(core.run), recoveryDelay: core.combat.recoveryDelay }, 0, 0);
let currentAction = currentDecision.action;
function update(dt: number): void {
  ...
  if (manual) {
    currentAction = manualAction();
    currentDecision = { action: currentAction, exploratory: false, qValue: 0 };
  } else {
    currentDecision = agent.decide(core.ship, core.enemies, core.enemyShots, core.orbs, { weapon: weaponKind(core.run), recoveryDelay: core.combat.recoveryDelay }, dt, lastReward);
