...
import { altitudeMargin, SEA_Y, stepPhysics, type GunshipBody } from './gunship_physics.js';
import { GunshipAgent } from './gunship_rl.js';
import { addXp, applyUpgrade, choicesFor, configureShip, createRunProgress, type GunshipUpgrade } from './gunship_progression.js';
...
const ui = { hp: $('hp'), hpText: $('hp-text'), wave: $('wave'), episode: $('episode'), action: $('action'), margin: $('margin'), burst: $('burst'), kills: $('kills'), reward: $('reward'), epsilon: $('epsilon'), states: $('states'), steps: $('steps'), deaths: $('deaths'), pause: $('pause'), mode: $<HTMLSelectElement>('mode'), level: $('level'), xp: $('xp'), upgrade: $('upgrade'), choices: $('upgrade-choices'), countdown: $('upgrade-countdown'), shipAccuracy: $('ship-accuracy'), airAccuracy: $('air-accuracy'), lowMargin: $('low-margin'), noseDown: $('nose-down'), cycle: $('cycle'), survival: $('survival'), data: $('data'), researchThrust: $<HTMLButtonElement>('research-thrust'), researchHull: $<HTMLButtonElement>('research-hull') };
...
let ship: GunshipBody = freshShip(); let agent = new GunshipAgent(); let enemies: Enemy[] = spawnWave(1, 1); let enemyShots: EnemyShot[] = []; let bullets: EnemyShot[] = [];
let wave = 1; let episode = 1; let kills = 0; let fallingDeaths = 0; let episodeReward = 0; let lastReward = 0; let paused = false; let restart = 0; let nextId = 50; let previous = performance.now(); let saveTimer = 0; let missileCooldown = 0; const run = createRunProgress(); let upgradeChoices: GunshipUpgrade[] = []; let upgradeTimer = 0;
let episodeElapsed = 0; let episodeMinMargin = Infinity; let noseDownSeconds = 0; let descentStarted = -1; let cycleTotal = 0; let cycleCount = 0; let shots = 0; let shipHits = 0; let airHits = 0; let shipShots = 0; let airShots = 0; const survivalHistory: number[] = [];
...
try { const saved = localStorage.getItem('gravity-gunship-q-v1'); if (saved) agent.restore(JSON.parse(saved)); } catch { /* A corrupt local save must not stop the flight loop. */ }
...
function frame(now: number): void { const dt = Math.min(.033, (now - previous) / 1000); previous = now; if (!paused) update(dt); resize(); renderGunship(ctx, canvas.width, canvas.height, ship, currentAction, enemies, enemyShots, bullets, wave); updateUi(); requestAnimationFrame(frame); }
let currentAction = agent.decide(ship, enemies, 0, 0).action;
function update(dt: number): void {
  if (restart > 0) { restart -= dt; if (restart <= 0) { resetTelemetry(); ship = freshShip(); enemies = spawnWave(wave, nextId); nextId += enemies.length; enemyShots = []; bullets = []; episode++; } return; }
  ...
  configureShip(ship, run);
  ship.thrustScale *= 1.06 ** meta.thrustResearch;
  currentAction = manual ? manualAction() : agent.decide(ship, enemies, dt, lastReward).action;
...
function resolveHits(): void {
  for (let i = bullets.length - 1; i >= 0; i--) { const bullet = bullets[i]; const target = enemies.find((enemy) => Math.hypot(enemy.x - bullet.x, enemy.y - bullet.y) < enemy.radius + 4); if (!target) continue; const surface = target.kind === 'ship' || target.kind === 'dreadnought'; if (surface) shipHits++; else airHits++; if (bullet.weapon !== 'laser') target.hp -= (bullet.weapon === 'missile' ? 48 : surface ? 14 : 24) * 1.2 ** run.damage; bullets.splice(i, 1); if (target.hp <= 0) { enemies.splice(enemies.indexOf(target), 1); kills++; const xp = target.kind === 'dreadnought' ? 12 : surface ? 3 : 1; episodeReward += target.kind === 'dreadnought' ? 18 : surface ? 5 : 2; addXp(run, xp); } }
...
function saveAgent(): void { try { localStorage.setItem('gravity-gunship-q-v1', JSON.stringify(agent.serialize())); localStorage.setItem('gravity-gunship-meta-v1', JSON.stringify(meta)); } catch { /* Storage may be unavailable in a private renderer. */ } }
function freshShip(): GunshipBody { const maxHp = Math.round(100 * 1.12 ** meta.hullResearch); return { x: 600, y: 260, vx: 0, vy: 0, angle: Math.PI / 2, hp: maxHp, maxHp, fireCooldown: 0, thrustScale: 1, turnScale: 1 }; }
...
function resetTelemetry(): void { episodeElapsed = 0; episodeMinMargin = Infinity; noseDownSeconds = 0; descentStarted = -1; cycleTotal = 0; cycleCount = 0; shots = 0; shipHits = 0; airHits = 0; shipShots = 0; airShots = 0; }
function processUpgrade(dt: number): void { if (!upgradeChoices.length) { upgradeChoices = choicesFor(run); upgradeTimer = 3; showUpgradeChoices(); } upgradeTimer -= dt; ui.countdown.textContent = `HUMAN WINDOW ${Math.max(0, upgradeTimer).toFixed(1)}s`; if (upgradeTimer <= 0) chooseUpgrade(upgradeChoices[0]); }
...
function chooseUpgrade(choice: GunshipUpgrade): void { applyUpgrade(run, choice); upgradeChoices = []; ui.upgrade.dataset.open = 'false'; }
