...
// ── Faction State ────────────────────────────────────────────────────────────
const factions = FACTION_DEFS.map(def => ({
    ...def,
    food: 65,
    material: 40,
    nodes: [],
...
for (let i = 0; i < factions.length; i++) {
    const base = map.nodes.reduce((b, n) => Math.hypot(n.x - CORNERS[i].x, n.z - CORNERS[i].z) <
...
const edgeMatFaction = factions.map(f => new THREE.LineBasicMaterial({ color: f.color, transparent: true, opacity: 0.55 }));
// ── Node Meshes ──────────────────────────────────────────────────────────────
const NODE_R = 3.6;
const BASE_R = 5.0;
...
const pulses = [];
function spawnPulse(fromNode, toNode, factionId) {
...
function tickPulses(dt) {
    for (let i = pulses.length - 1; i >= 0; i--) {
        ...
        p.mesh.position.lerpVectors(new THREE.Vector3(p.from.x, 0.5, p.from.z), new THREE.Vector3(p.to.x, 0.5, p.to.z), p.t);
    }
}
// ── World State ──────────────────────────────────────────────────────────────
const world = { elapsed: 0, tickTimer: 0, tick: 0, eventTimer: rng.range(18, 34) };
// ── Faction Rules ────────────────────────────────────────────────────────────
const DEFAULT_RULES = {
    builder: [
...
const factionRules = {};
for (const f of factions)
    factionRules[f.id] = [...(DEFAULT_RULES[f.personality] ?? [])];
    ...
    for (const f of factions) {
        try {
            ...
            if (Array.isArray(data.rules))
                factionRules[f.id] = data.rules;
        }
        catch (_) { }
    }
}
// ── Snapshot for Rule Evaluation ─────────────────────────────────────────────
function buildSnap(faction) {
...
function largestCluster(factionId) {
    ...
    for (const start of owned) {
        ...
        while (queue.length) {
            ...
            for (const nb of cur.neighbors)
                if (!visited.has(nb.id) && nb.owner === factionId) {
                    visited.add(nb.id);
                    queue.push(nb);
                }
        }
...
function dominanceMult(faction) {
...
// ── Action Costs ──────────────────────────────────────────────────────────────
const FOOD_CAP = 80;
const COST = { expand: 18, attack: 28, fortify: 12, gather: 0 };
function execAction(faction, action) {
    if ((COST[action] ?? 0) > faction.food)
        return false;
    const owned = map.nodes.filter(n => n.owner === faction.id);
    const borders = owned.flatMap(n => n.neighbors).filter(n => n.owner !== faction.id);
    switch (action) {
        case 'expand': {
            ...
            if (src)
                spawnPulse(src, target, faction.id);
...
function applyInfluence(faction, target, power) {
    ...
    if (target.strength <= 0) {
        ...
        if (prev >= 0)
            factions[prev].nodes = factions[prev].nodes.filter(n => n.id !== target.id);
...
const RANDOM_EVENTS = [
    ...
    },
    {
        ...
        run() {
            ...
            for (const n of affected) {
                if (!n.isBase)
                    n.strength = Math.max(0, n.strength - rng.range(0.12, 0.28));
            }
    ...
    },
    {
        ...
        run() {
            ...
            if (dominant.n / map.nodes.length < 0.38)
                return;
            const borders = map.nodes.filter(n => n.owner === dominant.f.id && !n.isBase &&
...
function doIntervention(type) {
    switch (type) {
        ...
        case 'storm': {
            ...
            for (const n of affected)
                if (!n.isBase)
                    n.strength = Math.max(0, n.strength - 0.32);
...
function updateHUD() {
    ...
    if (el)
        el.textContent = `${mm}:${ss}  tick: ${world.tick}`;
}
