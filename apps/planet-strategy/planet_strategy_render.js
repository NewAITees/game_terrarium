...
export function createPlanetStrategyRenderer({ world, rng, getPlanet, distance3d, routeKey, }) {
    ...
    for (const empire of world.empires) {
        if (empire.homeFactoryId)
            homePlanetIds.add(empire.homeFactoryId);
        if (empire.homeMineId)
    ...
    function makeShipMesh(empire, kind = 'transport') {
        ...
        if (kind === 'transport') {
            ...
            const frame = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(1.16, 0.74, 2.96)), frameMaterial);
        ...
        if (kind === 'attacker') {
            ...
            hull.add(body);
            const frame = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(1.52, 1.52, 1.52)), new THREE.LineBasicMaterial({ color: 0xe6f4ff, transparent: true, opacity: 0.9 }));
            hull.add(frame);
    ...
    function ensureRouteVisual(route) {
        ...
        const glow = new THREE.Line(geometry.clone(), new THREE.LineBasicMaterial({ color: 0x9fe6ff, transparent: true, opacity: 0.08 }));
        routeGroup.add(line);
        routeGroup.add(glow);
        route.line = line;
        route.glow = glow;
    ...
    function updateVisuals(dt = 0) {
        tickDecorRings(dt);
        const contestedPlanets = new Set(world.ships
        ...
        for (const planet of world.planets) {
            ...
            if (planet.labelSprite) {
                ...
                if (planet.stalled)
                    ...
                    label = 'DEPLETED';
                else if (homePlanetIds.has(planet.id))
                    label = 'HOME';
        ...
        for (const ship of world.ships) {
            if (!ship.mesh)
                continue;
            if (ship.status === 'orbiting' || ship.status === 'battling')
                continue;
    ...
    function buildSceneObjects() {
        for (const planet of world.planets) {
            ...
            planet.mesh = mesh;
            planet.decorRings = [];
            const stockRing = new THREE.Mesh(new THREE.TorusGeometry(1.55, 0.07, 10, 36), new THREE.MeshBasicMaterial({ color: 0xb9e8ff, transparent: true, opacity: 0.22 }));
            ...
            const homeAura = new THREE.Mesh(new THREE.TorusGeometry(2.75, 0.11, 12, 48), new THREE.MeshBasicMaterial({ color: 0xdff8ff, transparent: true, opacity: 0.22 }));
            ...
            const alertRing = new THREE.Mesh(new THREE.TorusGeometry(2.15, 0.12, 10, 40), new THREE.MeshBasicMaterial({ color: 0xffd37a, transparent: true, opacity: 0 }));
            ...
            const oreRing = new THREE.Mesh(new THREE.TorusGeometry(1.95, 0.055, 8, 32), new THREE.MeshBasicMaterial({ color: 0xffcc44, transparent: true, opacity: 0.55 }));
            ...
            if (!isOwned)
                continue;
            if (planet.type === 'mine') {
                const ring = new THREE.Mesh(new THREE.TorusGeometry(2.2, 0.06, 8, 32), new THREE.MeshBasicMaterial({ color: new THREE.Color(empire.color), transparent: true, opacity: 0.6 }));
                ...
                for (let i = 0; i < 3; i++) {
                    const shard = new THREE.Mesh(new THREE.TetrahedronGeometry(0.34 + i * 0.05, 0), new THREE.MeshBasicMaterial({ color: 0xffcf68, transparent: true, opacity: 0.78 }));
            ...
            if (planet.type === 'factory') {
                for (let i = 0; i < 2; i++) {
                    const ring = new THREE.Mesh(new THREE.TorusGeometry(2.3 + i * 1.0, 0.07, 8, 34), new THREE.MeshBasicMaterial({ color: new THREE.Color(empire.color), transparent: true, opacity: 0.55 }));
                    ring.position.set(planet.x, planet.y, planet.z);
                ...
                scene.add(light);
                const beacon = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.22, 7.5, 10), new THREE.MeshBasicMaterial({ color: 0xa8ecff, transparent: true, opacity: 0.28 }));
    ...
    function updateTrail(ship) {
        if (!ship.trailLine || !ship.mesh)
            return;
        const point = ship.mesh.position.clone();
        ship.trailPoints.push(point);
        while (ship.trailPoints.length > 10)
            ship.trailPoints.shift();
        if (ship.trailPoints.length < 2)
            return;
        ship.trailLine.geometry.dispose();
...
function addStarField(scene, rng) {
    ...
    scene.add(new THREE.Points(geo, new THREE.PointsMaterial({ vertexColors: true, size: 1.2, transparent: true, opacity: 0.58, sizeAttenuation: true })));
}
