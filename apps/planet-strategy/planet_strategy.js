...
function generatePlanetPositions(count, options) {
    const positions = [];
    const { minRadius = 60, maxRadius = 260, minDistance = 60, verticalRange = 90, maxAttempts = 80, } = options;
    for (let i = 0; i < count; i++) {
        let accepted = null;
...
function runMining(step) {
    ...
    for (const planet of world.planets) {
        if (planet.owner < 0 || planet.resources <= 0 || planet.structures.mine <= 0)
            continue;
        ...
        if (planet.resources <= 0)
            maybeLog(`depleted:${planet.id}`, `${planet.label} depleted its ore veins.`, 'warning', 1);
    }
}
    ...
    for (const ship of world.ships) {
        ...
        if (ship.status === 'loading') {
            ...
            if (to.underConstruction?.empireId === ship.owner) {
                ...
                if (to.underConstruction.progress >= to.underConstruction.needed)
                    completeConstruction(to);
            }
            else {
                to.stock += amount;
...
function queueConstruction(empire, planet, type) {
    if (planet.underConstruction || planet.owner >= 0)
        return;
...
function assignRoutes() {
    for (const empire of world.empires) {
        ...
        if (!mines.length || !factories.length)
            continue;
    ...
    for (const empire of world.empires) {
        ...
        for (const factory of ownedFactories) {
            ...
            if (factory.stock >= FACTORY_MAINTENANCE_COST) {
                ...
                if (factory.stalled)
                    maybeLog(`resume:${factory.id}`, `${factory.label} recovered from ore starvation.`, 'resource', 6);
                factory.stalled = false;
                if (isHome)
                    empire.stalledTime = Math.max(0, empire.stalledTime - step * 0.5);
            }
            else {
                factory.stalled = true;
                if (isHome)
                    empire.stalledTime += step;
...
function updateShips(dt) {
    for (const ship of world.ships) {
        if (ship.status !== 'travel' && ship.status !== 'travel_back')
            continue;
...
function updateOrbiting(dt) {
    for (const ship of world.ships) {
        ...
        ship.mesh.position.set(planet.x + Math.cos(ship.orbitAngle) * r, planet.y + 2 + ship.owner * 1.8, planet.z + Math.sin(ship.orbitAngle) * r);
        ...
        const intensity = ship.status === 'battling'
            ...
            : 0.25 + (1 - hpFrac) * 0.3;
        ship.mesh.traverse(node => { if (node.material && !Array.isArray(node.material))
...
function decideAttacks() {
    for (const empire of world.empires) {
        const myAttackers = world.ships.filter(s => s.owner === empire.id && s.kind === 'attacker' && s.status === 'orbiting');
        if (myAttackers.length < 3)
            continue;
        ...
        const base = getPlanet(empire.homeFactoryId)?.owner === empire.id
            ...
            : ownedPlanets[0] ?? null;
        const enemyPlanets = world.planets.filter(p => p.owner !== empire.id &&
        ...
        const scored = enemyPlanets.map(p => {
            const defCount = world.ships.filter(s => s.homePlanetId === p.id && s.kind !== 'transport' &&
                (s.status === 'orbiting' || s.status === 'battling')).length;
        ...
        if (base)
            touchRoute(base.id, target.id, 8);
...
function runCombat(dt) {
    ...
    // 惑星ごとに戦闘解決
    const contested = new Set(world.ships.filter(s => s.status === 'battling').map(s => s.targetPlanetId));
    for (const planetId of contested) {
        ...
        if (attackers.length && planet.owner === attackers[0].owner) {
            for (const s of attackers) {
                ...
                s.homePlanetId = planetId;
            }
            continue;
        }
        const defenders = world.ships.filter(s => s.owner === planet.owner && s.homePlanetId === planetId &&
        ...
        // 撃破
        const killed = world.ships.filter(s => s.hp <= 0 && s.kind !== 'transport' &&
        ...
        const stillDef = world.ships.filter(s => s.owner === planet.owner && s.homePlanetId === planetId &&
            s.kind !== 'transport' && (s.status === 'orbiting' || s.status === 'battling'));
...
function evaluateEmpireCollapse() {
    for (const empire of world.empires) {
        ...
        if (factory && factory.owner !== empire.id)
            collapseEmpire(empire, 'lost its factory planet');
        else if (ownedPlanets <= 0)
            collapseEmpire(empire, 'lost all planets');
        else if (transports <= 0)
            collapseEmpire(empire, 'lost all transport ships');
...
function maybeLog(key, text, type, intervalSeconds) {
    ...
    if (world.time - last < intervalSeconds)
        return;
