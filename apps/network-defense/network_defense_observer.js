...
import { RNG, STYLE, buildAdj, buildEdges, buildScene, buildTopology, edgeKey, findShortestPath, makeMats, tickEdges, } from './network-core.js';
import { applyPersonalitiesToRules, applyPersonalityToAgent, buildObserverSnapshot, pickRankPersonalities, } from './network_defense_personality.js';
...
const WIN_WAVE = 10;
const RULE_UPDATE_RANKS = ['senior', 'mid', 'junior'];
...
const game = {
    ...
    lowLoadMode: false,
    telemetryCooldown: 0,
    rankIntents: Object.fromEntries(Object.entries(rankPersonalities).map(([rank, personality]) => [rank, personality.summary])),
};
const enemyPackets = [];
...
function enemyFrontierTarget() {
    ...
    for (const node of infected) {
        for (const neighbor of adj.get(node.id) || []) {
            if (neighbor.rebootUntil > performance.now() / 1000)
                continue;
            if (neighbor.isServer && !exposedServer())
                continue;
            if (neighbor.infection < 0.72)
...
function safeRoute(from, to) {
    ...
    while (queue.length) {
        ...
        for (const neighbor of adj.get(current.id) || []) {
            if (visited.has(neighbor.id) || !isFriendlyPassable(neighbor, to))
                continue;
...
function createPacket(color, radius) {
    const mesh = new THREE.Mesh(new THREE.SphereGeometry(radius, 9, 9), new THREE.MeshBasicMaterial({ color }));
    scene.add(mesh);
    return mesh;
}
...
function createAgent(rank, index = agents.length) {
    ...
    const mesh = new THREE.Mesh(new THREE.OctahedronGeometry(spec.size, 0), new THREE.MeshBasicMaterial({ color: spec.color }));
...
function deployFirewall(edge, now) {
    ...
    group.add(glow);
    const ring = new THREE.Mesh(new THREE.TorusGeometry(1.55, 0.09, 8, 22), new THREE.MeshBasicMaterial({
        color: 0x77dfff,
        transparent: true,
        opacity: 0.45,
        blending: THREE.AdditiveBlending,
...
function sendAgent(agent, target, blockedTarget = null) {
    ...
    if (stats.cost > 0 && game.credits < stats.cost)
        return false;
...
async function triggerRuleUpdate() {
    ...
    if (statusEl)
        statusEl.textContent = `rules: asking ollama for ${rank}…`;
    try {
        ...
        if (statusEl)
            statusEl.textContent = `rules: ollama rewrote ${rank} (${data.ruleCount} rules)`;
        ...
        if (statusEl)
            statusEl.textContent = `rules: update failed — ${err.message}`;
...
async function loadAgentRules() {
    ...
    if (anyLoaded) {
        ...
        if (el)
            el.textContent = `rules: loaded ${rulesLoadedAt}`;
    }
}
    ...
    for (const [key, val] of Object.entries(when)) {
        switch (key) {
            case 'serverNeighborInfection':
                if (snap.serverNeighborMaxInfection <= val)
                    return false;
                break;
            case 'hottestInfection':
                if (snap.hottestInfection <= val)
                    return false;
                break;
            case 'avgInfection':
                if (snap.avgInfection <= val)
                    return false;
                break;
            case 'enemyCount':
                if (snap.enemyCount < val)
                    return false;
                break;
            case 'infectedCount':
                if (snap.infectedCount < val)
                    return false;
                break;
            case 'serverHpBelow':
                if (snap.serverHp >= val)
                    return false;
                break;
            case 'waveGte':
                if (snap.wave < val)
...
function execAction(agent, action, snap) {
    switch (action) {
        ...
        case 'recruitMid': {
            ...
            if (game.credits < AGENT_COSTS.mid + res || now - (game.lastRecruitTime || 0) < cd)
                return false;
        ...
        case 'recruitJunior': {
            ...
            if (game.credits < AGENT_COSTS.junior + res || now - (game.lastRecruitTime || 0) < cd)
                return false;
            game.lastRecruitTime = now;
            buyAgent('junior');
            agent.cooldown = cd;
        ...
        case 'hardenNode': {
            ...
            if (!target || target.hardenUntil > now)
                return false;
        ...
        case 'rebootNode': {
            ...
            if (!target || target.rebootUntil > now)
                return false;
...
function runAgentRules(agent, snap) {
    for (const rule of agentRules[agent.rank] ?? []) {
        if (rule.when && !evalCondition(rule.when, snap))
            continue;
...
function spawnEnemy() {
    if (game.gameOver || game.waveRemaining <= 0)
        return;
    game.waveSpawned++;
...
function spawnScanner() {
    if (game.gameOver || scanPackets.length >= 10)
        return;
...
function updateScanPackets(dt) {
    for (let i = scanPackets.length - 1; i >= 0; i--) {
        ...
        for (let j = enemyPackets.length - 1; j >= 0; j--) {
            ...
            if (enemy.seg >= enemy.path.length - 1)
                continue;
...
function applyAttack(node, damage, now) {
    if (!node || game.gameOver || node.rebootUntil > now)
        return;
...
function setNodeColor(node, now) {
    ...
    if (node.hardenUntil > now)
        color.lerp(new THREE.Color(0x80e8ff), 0.55);
...
function updateNodes(dt, now) {
    ...
    for (const node of topo.nodes) {
        if (node.infection < 0.35 || node.rebootUntil > now)
            continue;
        for (const neighbor of adj.get(node.id) || []) {
            if (neighbor.isServer)
                continue; // サーバーは直接攻撃のみでダメージ
            if (neighbor.hardenUntil > now || neighbor.rebootUntil > now)
                continue;
        ...
        if (agent.t >= 1) {
            ...
            if (agent.seg >= agent.path.length - 1) {
                ...
                if ((agent.workCost ?? 0) > 0) {
                    if (game.credits < agent.workCost) {
                        idleAtSpot(agent);
                        continue;
                    }
...
function applyAgentArrival(agent, lastEdge, now) {
    ...
    switch (agent.arrivalAction) {
        ...
        default: {
            ...
            if (agent.blockedTarget || target.infection > 0.04)
                deployFirewall(frontierEdge, now);
            defensePackets.push({
...
function updateHud() {
    ...
    document.getElementById('buy-junior').disabled = game.credits < AGENT_COSTS.junior;
    document.getElementById('buy-mid').disabled = game.credits < AGENT_COSTS.mid;
...
function fallbackRule(snapshot) {
    if (snapshot.critical > 5 || snapshot.avgInfection > 0.45)
        return 'containment';
    if (snapshot.enemies > 3 || game.wave >= 4)
        return 'firewall-first';
    if (snapshot.infected < 2 && topo.server.hp > 90)
        return 'patrol';
    return 'balanced';
}
async function callLLM(snapshot) {
    try {
        ...
        if (!res.ok)
            throw new Error(`strategy ${res.status}`);
        const data = await res.json();
        if (data.rule)
            return data.rule;
    }
...
function toggleLowLoadMode(force) {
    ...
    if (next === game.lowLoadMode)
        return;
    game.lowLoadMode = next;
    game.telemetryCooldown = 0;
    applyRenderProfile();
    setMessage(next ? 'Low-load observation enabled.' : 'Full observation restored.');
    logEvent(next ? 'Observer mode switched to low-load rendering.' : 'Observer mode returned to full rendering.', 'summary');
}
function showEndOverlay(isVictory) {
    ...
    ].map(([lbl, val]) => `<div class="end-row"><span class="lbl">${lbl}</span><span class="val">${val}</span></div>`).join('');
...
function reportTelemetry() {
    ...
    window.Telemetry?.report('network_defense_observer', {
        ...
        observer: {
            ...
            personalities: Object.fromEntries(Object.entries(rankPersonalities).map(([rank, personality]) => [rank, personality.label])),
...
function interactNode(node) {
    ...
    if (game.mode === 'harden') {
        if (game.credits < 20) {
            setMessage('Need 20cr to harden a node.', true);
            return;
        }
        game.credits -= 20;
        ...
        if (game.credits < 40) {
            setMessage('Need 40cr to reboot a node.', true);
            return;
        }
        game.credits -= 40;
...
window.addEventListener('pointerdown', event => {
    ...
    if (hits.length)
        interactNode(hits[0].object.userData.node);
});
...
function animate() {
    ...
    for (const { mesh, s } of spinData) {
        ...
        if (s.rx)
            mesh.rotation.x += dt * s.rx;
        if (s.rz)
            mesh.rotation.z += dt * s.rz;
        if (s.ry)
            mesh.rotation.y += dt * s.ry;
        if (node?.rebootUntil > now)
    ...
    updatePackets(normalPackets, dt, node => { if (node)
        triggerFlash(normalPool, node); });
