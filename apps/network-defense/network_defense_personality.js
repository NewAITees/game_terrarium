...
function stablePrioritySort(rules, personality) {
...
export function pickRankPersonalities(rng) {
    return Object.fromEntries(Object.entries(PERSONALITY_LIBRARY).map(([rank, entries]) => [rank, rng.pick(entries)]));
}
export function applyPersonalitiesToRules(baseRules, rankPersonalities) {
    return Object.fromEntries(Object.entries(baseRules).map(([rank, rules]) => {
        const personality = rankPersonalities[rank];
        if (!personality)
            return [rank, rules.map(rule => ({ ...rule }))];
        return [rank, stablePrioritySort(rules, personality)];
    }));
}
export function applyPersonalityToAgent(agent, rankPersonalities) {
    const personality = rankPersonalities[agent.rank];
    agent.personality = personality;
    return agent;
}
export function describeAgentIntent(agent, fallbackIntent = 'observe the grid') {
    if (!agent)
...
export function buildObserverSnapshot(agents, rankPersonalities, rankIntents) {
    return ['senior', 'mid', 'junior'].map((rank) => {
