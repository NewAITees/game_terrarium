export function createObservationUi({ onToggleLowLoadMode, onIntervenePulse, onInterveneBreach, }) {
    const elements = {
    ...
    function update({ lowLoadMode, eventState, rankSnapshots, summary, hotspots }) {
        ...
        if (elements.eventName)
            elements.eventName.textContent = eventState.label;
        if (elements.eventDetail)
            elements.eventDetail.textContent = eventState.detail;
        if (elements.summaryText)
            elements.summaryText.textContent = summary.text;
        if (elements.summaryDetail)
            elements.summaryDetail.textContent = summary.detail;
        renderHotspots(hotspots);
        for (const entry of rankSnapshots)
            renderRankRow(entry);
    }
    return { update };
}
