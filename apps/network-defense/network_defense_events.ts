import type {
  ...
  NetworkDefenseObserverEventState,
  NetworkDefenseTopology,
} from '../../shared/types/network_defense.js';

export function createObservationEvents({
  game,
  topo,
  rng,
  logEvent,
  ...
  game: NetworkDefenseGameState;
  topo: NetworkDefenseTopology;
  rng: { next: () => number };
  logEvent: (text: string, type?: string) => void;
  setMessage: (message: string) => void;
}) {
  const state: NetworkDefenseObserverEventState = {
    active: null,
  ...
  function startSignalStorm(now: number) {
  ...
  function startMaintenanceWindow(now: number) {
  ...
  function startEvent(now: number) {
    if (rng.next() < 0.5) startSignalStorm(now);
    else startMaintenanceWindow(now);
  ...
  function applySignalStorm(active: NonNullable<NetworkDefenseObserverEventState['active']>, now: number, dt: number) {
  ...
  function applyMaintenanceWindow(active: NonNullable<NetworkDefenseObserverEventState['active']>, now: number, dt: number) {
  ...
  function update(dt: number, now: number) {
    if (state.active) {
      if (state.active.key === 'signal_storm') applySignalStorm(state.active, now, dt);
      if (state.active.key === 'maintenance_window') applyMaintenanceWindow(state.active, now, dt);
      state.remaining = Math.max(0, state.active.until - now);
      if (now >= state.active.until) endEvent();
      return;
    }

    state.remaining -= dt;
    if (state.remaining <= 0) startEvent(now);
  }
