import { BackSide,BufferGeometry,Line,LineBasicMaterial,LineDashedMaterial,Mesh,MeshBasicMaterial,MeshStandardMaterial,PointLight,QuadraticBezierCurve3,SphereGeometry,TorusGeometry,Vector3, } from 'three';
...
export function buildScene(topo: any, scene: any, spinData?: any[]) {
  for (const node of topo.nodes) {
    ...
    const mesh = new Mesh(style.geo(), mat);
    mesh.position.set(node.x, node.y, node.z);
    mesh.add(new Mesh(
      new SphereGeometry(style.halo, 12, 12),
      new MeshBasicMaterial({ color: style.color, transparent: true, opacity: style.hOp, side: BackSide })
    ));
    scene.add(mesh);
    node.mesh = mesh;
  ...
  for (let i = 0; i < 3; i++) {
    const ring = new Mesh(
      new TorusGeometry(2.6, .07, 6, 22),
      new MeshBasicMaterial({ color: 0xFFD060, transparent: true, opacity: .3 })
    );
  ...
  const serverGlow = new PointLight(0xFFD060, 3.5, 80);
...
export function makeMats() {
  return {
    tA: new LineBasicMaterial({ color: 0x88ddff, transparent: true, opacity: 1.0 }),
    tI: new LineBasicMaterial({ color: 0x0d1e33, transparent: true, opacity: 0.2 }),
    sA: new LineDashedMaterial({ color: 0xff8833, dashSize: 3, gapSize: 1, transparent: true, opacity: 1.0 }),
    sI: new LineDashedMaterial({ color: 0x401508, dashSize: 3, gapSize: 1, transparent: true, opacity: 0.45 }),
  };
}
...
export function buildEdges(topo: any, scene: any, edgeMap: Map<string, any>, allEdges: any[], mats: any) {
  function add(a: any, b: any, shortcut: boolean) {
    const p0 = new Vector3(a.x, a.y, a.z);
    const p2 = new Vector3(b.x, b.y, b.z);
    const mid = p0.clone().lerp(p2, .5);
    mid.y += shortcut ? 22 : 6;
    const curve = new QuadraticBezierCurve3(p0, mid, p2);
    const geo = new BufferGeometry().setFromPoints(curve.getPoints(70));
    const line = new Line(geo, shortcut ? mats.sI : mats.tI);
