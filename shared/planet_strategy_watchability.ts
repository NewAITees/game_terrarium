...
export function buildPlanetWatchState(world: any): { nextWatch: PlanetWatchCard; causal: PlanetCausalCard[] } {
  ...
  if (starved) {
    const owner = (world.empires ?? []).find((empire: any) => empire.id === starved.owner);
  ...
  const attacking = (world.ships ?? []).filter((ship: any) => ship.kind !== 'transport' && ['launching', 'traveling', 'approaching', 'engaging'].includes(ship.status) && ship.targetPlanetId);
  if (attacking.length) {
    const lead = attacking[0];
    const attacker = (world.empires ?? []).find((empire: any) => empire.id === lead.owner);
    const target = (world.planets ?? []).find((planet: any) => planet.id === lead.targetPlanetId);
  ...
  const contested = (world.planets ?? []).find((planet: any) => (world.ships ?? []).some((ship: any) => ship.status === 'engaging' && ship.targetPlanetId === planet.id));
  if (contested) {
    return {
