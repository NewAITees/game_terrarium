...
export function createPlanetStrategyRenderer({ world, rng, getPlanet, distance3d, routeKey }) {
  ...
  function makeShipMesh(empire, kind = 'transport') {
    ...
    if (kind === 'transport') {
      ...
      const frame = new THREE.LineSegments(
        ...
        new THREE.LineBasicMaterial({ color: 0xbfe2ff, transparent: true, opacity: 0.42 })
      );
      hull.add(frame);
      return hull;
    }
  ...
  function ensureRouteVisual(route) {
    ...
    const material = new THREE.LineBasicMaterial({ color: 0x5ca8c8, transparent: true, opacity: 0.18 });
  ...
  function updateVisuals(dt = 0) {
    ...
    for (const route of world.routes.values()) {
      ...
      route.line.material.opacity = 0.07 + Math.min(route.traffic / 18, 0.44);
      route.line.material.color.set(route.traffic > 8 ? 0x8adfff : 0x3f6d89);
    }

    for (const ship of world.ships) {
      if (!ship.mesh?.material) continue;
